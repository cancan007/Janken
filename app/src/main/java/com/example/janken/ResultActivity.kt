...
class ResultActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val id = intent.getIntExtra("MY_HAND", 0)   //様々なidを取得
    ...
    private fun saveData(myHand: Int, comHand: Int, gameResult: Int){
        val pref = PreferenceManager.getDefaultSharedPreferences(this)   //データ保存に使われるインスタンス
        ...
        editor.putInt("GAME_COUNT", gameCount + 1)   //自身に返すときは、チェーンで書ける
            .putInt("WINNING_STREAK_COUNT", edtWinningStreakCount)
            .putInt("LAST_MY_HAND", myHand)
            .putInt("LAST_COM_HAND", comHand)
            .putInt("BEFORE_LAST_COM_HAND", lastComHand)
            .putInt("GAME_RESULT", gameResult)
            .apply()   //applyしないと保存されない
    }
